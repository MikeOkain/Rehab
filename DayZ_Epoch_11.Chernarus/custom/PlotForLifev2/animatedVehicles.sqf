...
if (_inVehicle && {_vehicle isKindOf "AH1Z"}) then {
   ...
   if (ah1z_fold < 0) then {
     ...
     if !(isEngineOn theah1z) then {
       ah1z_fold = theah1z addAction ["Fold","custom\animate\ah1z_fold.sqf","",5,false,true];
       ah1z_unfold = theah1z addAction ["UnFold","custom\animate\ah1z_unfold.sqf","",5,false,true];
     };
   };
   if (isEngineOn theah1z) then {
