...
if ((speed player <= 1) && (!locked cursorTarget) && (cursorTarget isKindOf "CSJ_GyroC") && _canDo && ((player distance cursorTarget) < 5)) then {
	if (s_player_pack_Chopper < 0) then {
		s_player_pack_Chopper = player addaction[("<t color=""#FF0066"">" + ("Pack Gyro Chopper") +"</t>"),"custom\rc_menu\pack_Chopper.sqf","",5,false,true];
	};
} else {
...
if ((speed player <= 1) && (!locked cursorTarget) && (cursorTarget isKindOf "TT650_Civ") && _canDo && ((player distance cursorTarget) < 5)) then {
	if (s_player_pack_moto < 0) then {
		s_player_pack_moto = player addaction[("<t color=""#FF0066"">" + ("Pack Motorcycle") +"</t>"),"custom\rc_menu\pack_moto.sqf","",5,false,true];
	};
} else {
...
if ((speed player <= 1) && (cursorTarget isKindOf "Old_bike_TK_CIV_EP1") && _canDo && ((player distance cursorTarget) < 5)) then {
	if (s_player_pack_bike < 0) then {
		s_player_pack_bike = player addaction[("<t color=""#FF0066"">" + ("Pack Pedal Bike") +"</t>"),"custom\rc_menu\pack_bike.sqf","",5,false,true];
	};
} else {
...
//+++++Toggle Player Menu+++++
    if(speed player <= 1) then {
        if (s_player_toggle < 0) then {
            s_player_toggle = player addaction[("<t color=""#9900CC"">" + ("Player Menu") +"</t>"),"custom\misc\player_menu.sqf","",5,false,true,"", ""];
        };
		} else {
...
if (_inVehicle && (_vehicle isKindOf "AllVehicles") && (!Earplugs)) then {

    if (s_player_put_earplugs_on < 0) then {

        airvehicle = _vehicle;

        s_player_put_earplugs_on = airvehicle addAction ["Earplugs on","custom\earplugs\earplugs_on.sqf","",5,false,true];
...
if (_inVehicle && (_vehicle isKindOf "AllVehicles") && (Earplugs)) then {
    ...
    if (s_player_put_earplugs_off < 0) then {
        ...
        s_player_put_earplugs_off = airvehicle addAction ["Earplugs off","custom\earplugs\earplugs_off.sqf","",5,false,true];
...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
    ...
    _adminList = ["0","0","0"]; // Add admins here if you admins to able to manage all plotpoles
...
if((_typeOfCursorTarget in DZE_DoorsLocked)) then {
	if (s_player_manageDoor < 0) then {		 
     s_player_manageDoor = player addAction ["<t color='#0059FF'>Manage Door</t>", "custom\doorManagement\initDoorManagement.sqf", _cursorTarget, 5, false];
	};
} else {
	...
	*/
	...
	if (_isMan && !_isAlive && !_isAnimal) then {
		if (s_player_clothes < 0) then {
			s_player_clothes = player addAction [("<t color=""#0000FF"">" + ("Take Clothes") + "</t>"), "custom\misc\takeclothes.sqf",cursorTarget, 1, false, true, "",""];
	    };
	} else {
	...
	if ((dayz_myCursorTarget != _cursorTarget) && _isVehicle && !_isMan && _hasToolbox && (damage _cursorTarget < 1) && !_isDisallowRepair) then {
		if (s_player_repair_crtl < 0) then {
			...
			_menu1 = dayz_myCursorTarget addAction [localize "STR_EPOCH_PLAYER_SALVAGEV", "custom\misc\salvage_vehicle.sqf",_cursorTarget, 0, true, false, "",""];
	...
	if (!_isAlive and !_isZombie and !_isAnimal and _isMan and _canDo) then {
        if (s_player_bury_human < 0) then {
            s_player_bury_human = player addAction [format["Bury Body"], "custom\misc\bury_human.sqf",cursorTarget, 3, true, true, "", ""];
        };
    } else {
