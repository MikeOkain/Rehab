...
while {true} do {
	...
	if (!TFV_INWORK) then {
		if (([cursorTarget,TFV_TRADERDATA] call TFV_cursorTargetIsTrader) && ([TFV_TRADERDATA,cursorTarget] call TFV_humanityGetCanTrade) && ((player distance cursorTarget) < 5) && ((call TFV_getTotalCurrency) > 0) && (TFV_ACC) && ((call TFV_countCurrencyItems) > 1)) then {
			if (TFV_ADDACTION_3 == -1) then {TFV_ADDACTION_3 = player addAction [TFV_STR_COMPACTCURRENCY,"custom\TradeFromVehicle\system\cc.sqf","",9997,false,true]; TFV_TRADER = cursorTarget;};
		};
		...
		if ((vehicle player) != player) then {
			...
			if ((TFV_VEHICLE != objNull) && (TFV_ISOWNER)) then {
				if ((count(crew TFV_VEHICLE)) > 0) then {
					...
					if (([cursorTarget,TFV_TRADERDATA] call TFV_cursorTargetIsTrader) && ([TFV_TRADERDATA,cursorTarget] call TFV_humanityGetCanTrade) && ((player distance cursorTarget) < 5) && TFV_ATFV) then {
						if (TFV_ADDACTION_1 == -1) then {TFV_ADDACTION_1 = player addAction [(format [TFV_STR_TRADEFROMVEHICLE,[TFV_VEHICLE] call TFV_vehicleGetDisplayName]),"custom\TradeFromVehicle\system\menu.sqf","vehicle",9999,false,true]; TFV_TRADER = cursorTarget;};
					};
				};
			};
			if ((call TFV_unitHasBackpack) && ([cursorTarget,TFV_TRADERDATA] call TFV_cursorTargetIsTrader) && ((player distance cursorTarget) < 5) && TFV_ATFB) then {
				if (TFV_ADDACTION_2 == -1) then {TFV_ADDACTION_2 = player addAction [TFV_STR_TRADEFROMBACKPACK,"custom\TradeFromVehicle\system\menu.sqf","backpack",9998,false,true]; TFV_TRADER = cursorTarget;};
			};
		};
    };
