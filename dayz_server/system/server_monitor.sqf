...
_cpcimmune =["Plastic_Pole_EP1_DZ"];
// ### [CPC] Indestructible Buildables Fix
...
if (isServer && isNil "sm_done") then {
	...
	_totalvehicles = 0;
	{
		...
		if (_damage < 1) then {
			...
			if ((count _intentory > 0) && !(typeOf( _object) == "Plastic_Pole_EP1_DZ") && !(typeOf( _object) in  DZE_DoorsLocked)) then { // inteference with plot management script
